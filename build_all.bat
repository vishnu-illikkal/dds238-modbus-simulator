...
set GOOS=linux
set GOARCH=amd64
go build -ldflags="-s -w" -o bin\dds238-modbus-simulator-linux-amd64 .\cmd\meter-sim

echo [3/5] Building Linux (ARM64 / Raspberry Pi)...
set GOOS=linux
set GOARCH=arm64
go build -ldflags="-s -w" -o bin\dds238-modbus-simulator-linux-arm64 .\cmd\meter-sim

echo [4/5] Building macOS (Apple Silicon M1/M2/M3)...
set GOOS=darwin
set GOARCH=arm64
go build -ldflags="-s -w" -o bin\dds238-modbus-simulator-darwin-arm64 .\cmd\meter-sim

echo [5/5] Building macOS (Intel x86_64)...
set GOOS=darwin
set GOARCH=amd64
go build -ldflags="-s -w" -o bin\dds238-modbus-simulator-darwin-amd64 .\cmd\meter-sim

echo.
echo ==================================================================
echo [SUCCESS] All target binaries built successfully inside bin\:
